...
import { gameover, gamePhase, gameMotor, TowersPlacement, TowersAttacks, addActorsToWorld, TowersAttackRange, TreesPlacement } from "./actors.js";
import { ActorsTypeList } from "./defineType.js";
import { OptimalRoad } from "./optimal_road.js";
function DrawOptimalRoad(wCopy, Astar) {
    // let wCopy : world = w;
...
function DrawAttackRange() {
    let w = CreateWorld(20, 12);
    const start = Math.floor(w.Height / 2) * w.Width;
    const end = start - 1;
    ...
    w = TreesPlacement(w);
    const startPoint = { x: Math.floor(start / w.Width), y: start % w.Width };
    const endPoint = { x: Math.floor(end / w.Width), y: end % w.Width };
    const AstarRoad = OptimalRoad(startPoint, w, endPoint);
    // console.log(AstarRoad);
    w.Matrix[5][4].AnActor = ActorsTypeList.MagicTower;
    const TowersRange = TowersAttackRange({ x: 5, y: 4 }, w);
    display(w, end);
    console.log(TowersRange);
    // wCopy = TowersPlacement(wCopy);    
    for (let i = 0; i < TowersRange.length; ++i) {
